...
static void test_empty_model_list(void) {
  ...
  if (base_url != NULL) {
    ...
    assert(resp != NULL);
    liter_lm_assert_status(resp, 200L, "test_empty_model_list");
    liter_lm_assert_json_array_len(resp->body, "data", 0,
                                   "test_empty_model_list");
    liter_lm_response_free(resp);
  } else {
    /* Offline: assert against pre-recorded mock body. */
    liter_lm_assert_json_array_len(mock_body, "data", 0,
                                   "test_empty_model_list");
  }
}
...
static void test_list_models_error_401(void) {
  ...
  if (base_url != NULL) {
    ...
    LiterLmResponse *resp = liter_lm_http_get(url);
    assert(resp != NULL);
    liter_lm_assert_status(resp, 401L, "test_list_models_error_401");
    liter_lm_response_free(resp);
  } else {
    /* Offline: assert against pre-recorded mock body. */
