...
impl VertexOAuthCredentialProvider {
    ...
    /// # Errors
    ///
    /// Returns [`LiterLmError::Authentication`] if required fields are missing.
    pub fn from_service_account_json(json: &serde_json::Value) -> Result<Self, LiterLmError> {
        let email = json
            .get("client_email")
            .and_then(serde_json::Value::as_str)
            .ok_or_else(|| LiterLmError::Authentication {
    ...
    /// # Errors
    ///
    /// Returns [`LiterLmError::Authentication`] if the file cannot be read or
    /// parsed, or if required fields are missing.
    pub fn from_key_file(path: &Path) -> Result<Self, LiterLmError> {
        let contents = std::fs::read_to_string(path).map_err(|e| LiterLmError::Authentication {
            message: format!("failed to read service account key file {}: {e}", path.display()),
        })?;

        let json: serde_json::Value = serde_json::from_str(&contents).map_err(|e| LiterLmError::Authentication {
    ...
    /// # Errors
    ///
    /// Returns [`LiterLmError::Authentication`] if the environment variable is
    /// not set, the file cannot be read, or it cannot be parsed.
    pub fn from_env() -> Result<Self, LiterLmError> {
        let path = std::env::var("GOOGLE_APPLICATION_CREDENTIALS").map_err(|_| LiterLmError::Authentication {
    ...
    async fn fetch_token(&self) -> Result<CachedToken, LiterLmError> {
        let now = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map_err(|e| LiterLmError::Authentication {
        ...
        let encoding_key = EncodingKey::from_rsa_pem(self.private_key_pem.expose_secret().as_bytes()).map_err(|e| {
            LiterLmError::Authentication {
                message: format!("invalid RSA private key: {e}"),
            }
        })?;

        let assertion =
            jsonwebtoken::encode(&header, &claims, &encoding_key).map_err(|e| LiterLmError::Authentication {
        ...
        let body = resp.text().await.map_err(|e| LiterLmError::Authentication {
            message: format!("Vertex OAuth token response unreadable: {e}"),
        })?;

        if !status.is_success() {
            return Err(LiterLmError::Authentication {
                message: format!("Vertex OAuth token request returned {status}: {body}"),
            });
        }

        let parsed: TokenResponse = serde_json::from_str(&body).map_err(|e| LiterLmError::Authentication {
