...
//! | `AWS_ROLE_SESSION_NAME` | Session name (defaults to `liter-lm-session`) |
...
const DEFAULT_SESSION_NAME: &str = "liter-lm-session";
...
impl WebIdentityCredentialProvider {
    ...
    /// # Errors
    ///
    /// Returns [`LiterLmError::Authentication`] if `AWS_ROLE_ARN` or
    ...
    async fn fetch_credentials(&self) -> Result<CachedCredentials, LiterLmError> {
...
fn parse_sts_response(xml: &str) -> Result<StsCredentials, LiterLmError> {
...
fn extract_xml_element(xml: &str, tag: &str) -> Result<String, LiterLmError> {
    let open = format!("<{tag}>");
    let close = format!("</{tag}>");

    let start = xml.find(&open).ok_or_else(|| LiterLmError::Authentication {
        message: format!("STS response missing <{tag}> element"),
    })? + open.len();

    let end = xml[start..].find(&close).ok_or_else(|| LiterLmError::Authentication {
        message: format!("STS response missing </{tag}> element"),
    })? + start;
...
fn env_var_required(name: &str) -> Result<String, LiterLmError> {
    std::env::var(name).map_err(|_| LiterLmError::Authentication {
